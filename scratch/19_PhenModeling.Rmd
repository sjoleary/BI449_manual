...
```{r fig.cap="Observed (orange) and corresponding predicted (blue) flowering dates using linear regression model (black line) for American Elm in Ramsey Country.", echo=FALSE}
...
Because all the outputs are tibbles, we can easily combine them all into a single dataframe using `bind_cols()`:
...
![Comparison of underfit (left), optimal (middle), or overfit (right) model.](figures/overfitting.png)
...
The panel on the right illustrates what happens when we **overfit** the model to the data. Here, the model fits the ata almost perfectly - which means that it describes the noise not the general pattern in the data.
...
Because we are randomly sub-setting our data set we need to set our initial seed number which means that every time we run the analysis the same set of random numbers will be generated.
...
```{r fig.cap="Comparison of predicted vs. observed flowering date for training data set. The dashed black line indicates perfect predictions, the red line is the linear trendline showing relationship of observed vs predicted values."}
...
In general, a smaller RMSE indicates a better model fit.
...
Generally, the next step would be the process of **Cross-validation** where we would repeatedly split our data set into training and test data sets to see
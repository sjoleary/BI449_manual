...
As always, let's start by loading our packages.
...
For example, Carlson and Baremore 2005 determine the following length/history stage relationships for spinner sharks (*C. brevipinna*)
...
First, conceptually describe how you cpuld add this information to your data sheet in excel as a new column called `Estimated_Stage`.
...
The second method circumvents having to first create subsets of the initial `data.frame` using something called a "conditional mutate".
...
`r msmbstyle::question(label = "ques:shn-2", text = "Use this data set to create a table with the number of individuals per life history stage caught at each site.")`
...
`r msmbstyle::question(label = "ques:shn-2", text = "Briefly describe your results to compare total catch across sites accounting for differences in life history stage.")`

Specifically for YOY let's take a look on whether we catch them across months.
...
`r msmbstyle::question(label = "ques:shn-2", text = "Consider disadvantages of using absolute counts of occurence to compare composition across sites. What measure could you use instead of absolute abundance to fix this issue?")`
...
`r msmbstyle::question(label = "ques:shn-3", text = "Briefly discuss what measures we could use to determine effort.")`
...
`r msmbstyle::question(label = "ques:shn-2", text = "Take a quick look at the data set to determine what columns are included and what information we can learn about each individual set. How can you amend the data set to include hook hours?")`
...
`r msmbstyle::question(label = "ques:shn-2", text = "If we look at our elasmo data.frame you will notice that we have a column called Set but that number indicates the nth set of a give sample day. How can you add a column called Set_ID that consists of the date and the set number?")`
...
`r msmbstyle::question(label = "ques:shn-2", text = "Now create a new object called elasmos_set that contains the number of sharks caught per set.")`
...
`r msmbstyle::question_begin(label = "ques:shn-2")`
...
Note the notation `elasmo_set <- left_join(elasmos_set, set_meta, by = "Set_ID")` will produce the same result.
...
`r msmbstyle::question(label = "ques:shn-2", text = "Briefly describe your results and discuss what this result could mean for our overarching question of identifying shark nurseries.")`

Of course, we are interested how CPUE compares across species and sites.

`r msmbstyle::question(label = "ques:shn-2", text = "Choose one species and calculate the CPUE per set. For convenience convert CPUE to effort per 1000 hook hours and then calculate the mean CPUE per 1000 hooks per site for that species.")`
...
`r msmbstyle::question(label = "ques:shn-2", text = "Briefly describe the results comparing CPUE per species and site and relate that to our overarching question of identifying shark nurseries; you may need to run additional pairwise tests (Dunn's test) to determine significant differences.")`
...
`r msmbstyle::question(label = "ques:shn-2", text = "Briefly describe the results comparing CPUE per life history stage and site and relate that to our overarching question of identifying shark nurseries; these results are all statistically significant.")`

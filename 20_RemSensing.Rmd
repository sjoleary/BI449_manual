...
Use the scatte rplots to make predictions about statistical relationships. Consider whether any of these plots look like they are visualizing strong relationships.
...
sites_pears <- sites %>%
  left_join(tidy_cor, by = c("site_id" = "Site")) %>%
...
ggplot() +
  ...
  geom_text(data = sites_pears, aes(x = longitude, y = latitude,    # add sites as labels
                              label = site_id, color = "darkblue"),
            size = site_size) +
  geom_text(data = sites_pears, aes(x = longitude, y = latitude,    # add pearson coefficient as labels
                              label = round(pearson, digits = 2), color = "darkred"),
            size = cor_size, nudge_y = -1) +                # shift label below actual coordinates
  scale_color_manual(values = c("darkblue", "darkred")) +

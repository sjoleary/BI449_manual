...
* **Mapping** or **Aesthetics** including variables to be plotted, plot positions (e.g. x or y axis), and encodings such as size, shape, color^[this is specified as the `mapping` argument using `aes()`. Again you can explicitly call the argument as `mapping = `, i.e. `ggplot(data = df, mapping = aes())`, or you can just specify the mapping aesthethics using `aes()` because you are using the arguments in the correct sequence `(ggplot(df, aes()))`.].
* **Geometric objects** (or layers) are what you actually end up seeing in the visualization, e.g. points, lines, bars etc. to represent data. This can also include statistical transformations^[these functions will all start with `geom_*` or `stat_*`].
...
Additionally, you can create and use **themes** that control additional aspects of your your figure is displayed including font size, background color, etc.
